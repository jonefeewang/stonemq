...
macro_rules! array_of {
    ($type:ty) => {
        pub fn array_of(value: $type) -> DataType {
...
impl TryFrom<DataType> for ValueSet {
    ...
    fn try_from(value: DataType) -> Result<Self, Self::Error> {
        return match value {
            DataType::SchemaValues(values) => Ok(values),
...
impl From<Acks> for DataType {
    fn from(value: Acks) -> Self {
        match value {
            Acks::All(value, _) => value.into(),
            Acks::Leader(value, _) => value.into(),
            Acks::None(value, _) => value.into(),
        }
    }
}
impl From<&Acks> for DataType {
...
impl From<&ApiKey> for DataType {
    fn from(value: &ApiKey) -> Self {
        match value {
            ApiKey::Produce(value, _) => (*value).into(),
            ApiKey::Metadata(value, _) => (*value).into(),
        }
    }
}
...
impl DataType {
    ...
    pub fn array_of_value_set(values: Vec<DataType>, schema: Arc<Schema>) -> DataType {
        DataType::Array(ArrayType {
            can_be_empty: false,
            p_type: Arc::new(DataType::SchemaValues(ValueSet::new(schema))),
            values: Some(values),
        })
    }
    ///
...
impl From<&ApiVersion> for DataType {
    fn from(value: &ApiVersion) -> Self {
        match value {
            ApiVersion::V0(value) => (*value).into(),
            ApiVersion::V1(value) => (*value).into(),
            ApiVersion::V2(value) => (*value).into(),
            ApiVersion::V3(value) => (*value).into(),
        }
    }
}
