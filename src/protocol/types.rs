...
impl DataType {
    pub fn size(&self) -> AppResult<i32> {
        return match self {
            DataType::Bool(bool) => Ok(bool.wire_format_size() as i32),
            DataType::I8(i8) => Ok(i8.wire_format_size() as i32),
            DataType::I16(i16) => Ok(i16.wire_format_size() as i32),
            DataType::I32(i32) => Ok(i32.wire_format_size() as i32),
            DataType::U32(u32) => Ok(u32.wire_format_size() as i32),
            DataType::I64(i64) => Ok(i64.wire_format_size() as i32),
            DataType::PString(string) => Ok(string.wire_format_size() as i32),
            DataType::NPString(nstring) => Ok(nstring.wire_format_size() as i32),
            DataType::PBytes(bytes) => Ok(bytes.wire_format_size() as i32),
            DataType::NPBytes(nbytes) => Ok(nbytes.wire_format_size() as i32),
            DataType::PVarInt(varint) => Ok(varint.wire_format_size() as i32),
            DataType::PVarLong(varlong) => Ok(varlong.wire_format_size() as i32),
            DataType::Array(array) => Ok(array.size()? as i32),
            DataType::Records(records) => Ok(records.wire_format_size() as i32),
