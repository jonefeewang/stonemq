...
impl Iterator for MemoryRecords {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(batch_size) = batch_size {
            ...
            // 使用 split_to 临时分割出一个批次，8个字节是base_offset+ length 自身长度
            let batch_buffer = buffer.split_to(batch_size + 8 + 4);
...
impl Default for RecordBatchBuilder {
    fn default() -> Self {
        let mut record_batch = RecordBatchBuilder {
...
impl RecordBatchBuilder {
    ...
    pub fn build(&mut self) -> RecordBatch {
        ...
        RecordBatch::new(record_batch_buffer)
    }
