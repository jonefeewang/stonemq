...
impl CheckPointFile {
    ...
    pub async fn read_checkpoints(&self) -> AppResult<HashMap<TopicPartition, i64>> {
        ...
        reader.read_line(&mut line_buffer).await.map_err(|e| {
...
mod tests {
    ...
    async fn test_write_and_read_checkpoints() -> AppResult<()> {
        ...
        checkpoint_file.write_checkpoints(points.clone()).await.unwrap();

        let read_points = checkpoint_file.read_checkpoints().await.unwrap();
